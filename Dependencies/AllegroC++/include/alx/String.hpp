...
public:
    ...
    void toBuffer(char *buffer, size_t size) const {
        al_ustr_to_buffer(get(), buffer, static_cast<int>(size));
    }
    ...
    int findReverse(uint32_t cp, int offset = 0) const {
        return al_ustr_rfind_chr(get(), offset >= 0 ? offset : static_cast<int>(al_ustr_size(get())), cp);
    }
    ...
    int findReverse(const String &str, int offset = -1) const {
        return al_ustr_rfind_str(get(), offset >= 0 ? offset : static_cast<int>(al_ustr_size(get())), str.get());
    }
    ...
    String(const String &str, size_t index, size_t len = -1) :
        Shared(al_ustr_dup_substr(str.get(), al_ustr_offset(str.get(), static_cast<int>(index)), al_ustr_offset(str.get(), (int)len >= 0 ? int(index + len) : INT_MAX)), al_ustr_free)
    {
    }
    ...
    const_iterator end() const {
        return const_iterator(get(), static_cast<int>(al_ustr_size(get())));
    }
     ...
     */
    iterator end() {
        return iterator(get(), static_cast<int>(al_ustr_size(get())));
    }
    ...
    int32_t operator [](size_t index) const {
        return al_ustr_get(get(), al_ustr_offset(get(), static_cast<int>(index)));
    }
    ...
    CodePointRef operator [](size_t index) {
		return CodePointRef(get(), al_ustr_offset(get(), static_cast<int>(index)));
    }
     ...
     */
    int32_t back() const {
        int offset = static_cast<int>(al_ustr_size(get()));
    ...
    String &append(const String &str, size_t offset, size_t size) {
        ...
        const ALLEGRO_USTR *substr = al_ref_ustr(&info, str.get(), static_cast<int>(offset), static_cast<int>(offset + size));
