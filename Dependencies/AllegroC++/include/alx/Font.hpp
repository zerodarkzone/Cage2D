...
public:
	...
	/**
		...
		@param flags font flags.
	 */
	Font(const File &file, const std::string &filename, int size, int flags = 0) :
		Shared(al_load_ttf_font_f(file.get(), filename.c_str(), size, flags), al_destroy_font)
	{
	}
	...
	/**
		...
		@return true on success.
	 */
	bool load(const File &file, const std::string &filename, int size, int flags = 0) {
		reset(al_load_ttf_font_f(file.get(), filename.c_str(), size, flags), al_destroy_font);
		return (bool)(*this);
	}
	...
	bool grab(const Bitmap &bmp, const std::vector<std::tuple<int, int>> &ranges) {
		...
		reset(al_grab_font_from_bitmap(bmp.get(), (int)rangeBuffer.size(), rangeBuffer.data()), al_destroy_font);
		return (bool)(*this);
	}
