...
namespace Cage2d
{
	Body::Body( bool solid ) : _vel( 0.0, 0.0 ), _accel( 0.0, 0.0 ), _iVel( 0.0, 0.0 ),
		...
		_groupMask( UINT16_MAX ), _collisionMask( UINT16_MAX ), _draw( false ), _dirX( 0 ), _dirY( 0 ), _type( Body::Type::STATIC )
	{
	...
	void Body::Update( )
	{
		...
		if( _entity )
		{			
			if( _type == Type::KINEMATIC )
			{				
				_rect.setCenter( _entity->GetPosition( ) );
			}
	...
	bool Body::Collision( const Body& other )
	{
		...
		if( other.GetType( ) == Type::STATIC /*Testing*/ || other._dirX == _dirX || other._dirY == _dirY )
		{
			_otherStatic = true;
		}
		_collision = false;
		...
		{
			if( _entity && other._entity && _rect.intersects( other._rect ) && ( _collision = _entity->HandlePreContactCollision( other._entity ) ) )
			{
				bool _occollision = false;
				if( _otherStatic && ( ( other._collisionMask & _groupMask ) != 0 ) )
				{
					_occollision = other._entity->HandlePreContactCollision( _entity );
				}
				_collision = true;
